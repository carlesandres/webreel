...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Default source-frame capture settings for all videos</td>
    </tr>
    <tr>
      <td>
        <code>theme</code>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>inherited</td>
      <td>
        Source-frame capture settings. Supports <code>format: "jpeg" | "png"</code>
      </td>
    </tr>
    <tr>
      <td>
...
Use `capture.format` to control how source frames are captured before final video encoding.

- `jpeg` is the default and preserves current behavior
- `png` is opt-in and preserves more detail before ffmpeg encodes the final output

`capture` can be set at the top level as a default and overridden per video:
...
PNG can improve text and UI sharpness in the source frames, but may increase capture CPU usage, ffmpeg pipe bandwidth, and frame storage size when using `webreel record --frames`.
